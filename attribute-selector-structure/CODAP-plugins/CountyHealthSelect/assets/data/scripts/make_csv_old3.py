...
# Sorted by length descending so that multi-word states are matched first.
states = sorted([
...
def read_sheet_with_header(excel_file, sheet_name):
    # Always use row 1 (header=1) as the header row.
...
def process_excel_file(file_path, state, year, output_csv_dir):
    ...
    try:
        df_select = read_sheet_with_header(file_path, select_sheet)
        df_additional = read_sheet_with_header(file_path, additional_sheet)
    except Exception as e:
        print(f"Error reading sheets from {file_path}: {e}")
        return

    # For Alaska and Louisiana, rename the key column.
    if state == "alaska" and "Borough" in df_select.columns:
        df_select.rename(columns={"Borough": "County"}, inplace=True)
    if state == "alaska" and "Borough" in df_additional.columns:
        df_additional.rename(columns={"Borough": "County"}, inplace=True)
    if state == "louisiana" and "Parish" in df_select.columns:
        df_select.rename(columns={"Parish": "County"}, inplace=True)
    if state == "louisiana" and "Parish" in df_additional.columns:
        df_additional.rename(columns={"Parish": "County"}, inplace=True)

    # Fallback for missing FIPS: rename "FIPS Code" to "FIPS".
    for df in [df_select, df_additional]:
