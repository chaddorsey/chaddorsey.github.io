...
# Updated candidate source columns for each target column.
target_candidates = {
    "FIPS": ["FIPS", "FIPS Code"],
    ...
    "Youth Not in School or Employment (%)": [
        ...
        "Youth Not in School or Employment",
        "Youth Not in School or Employment (%)",
        "Youth Not in School or Employment_y",
        "Not in School or Employment (%)"
    ]
}
...
def process_excel_file(file_path, state, year, output_csv_dir):
    ...
    # For "select" data, prefer "Select Measure Data"; else "Ranked Measure Data".
    ...
    # For Alaska and Louisiana, rename "Borough" or "Parish" to "County".
    if state == "alaska":
    ...
    # Fallback: if "FIPS" is missing but "FIPS Code" exists, rename it.
    for df in [df_select, df_additional]:
    ...
    output_filename = f"{year}_county_health_rankings_{state}.csv"
